...
const APIService = {
  ...
  async updateFCMToken(chefId, fcmToken) {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/chefs/update-fcm-token`,
        {
          chef_id: chefId,
          fcm_token: fcmToken,
        }
  ...
  async getChefStatus(chefId) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/chefs/status/${chefId}`
      );
  ...
  async fetchCurrentInstantOrderByChefId(chef_id) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/orders/instant/current`,
        {
          params: { chef_id },
        }
      );
  ...
  async fetchBookedDates(chef_id, date) {
    try {
      const response = await axios.get(`${API_BASE_URL}/orders/blocked-dates`, {
        params: {
          chef_id: chef_id,
          date: date,
        },
  ...
  async bookAdvancedOrder(
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/orders/advance`, {
        chef_id: chef_id,
  ...
  async fetchCompletedOrders(chef_id) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/orders/completed-orders/${chef_id}`
      );
      return response.data;
    } catch (error) {
  ...
  async fetchCustomerOrders(customer_id) {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/orders/customer-orders/${customer_id}`
      );
      return response.data;
    } catch (error) {
      console.error("Error fetching completed orders:", error);
  ...
  async updateInstantBookStatus(orderId, chef_id, status) {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/orders/update-instant-book`,
        {
          orderId,
          chef_id,
          status,
  ...
  async confirmAdvanceBooking(order_id) {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/orders/advance/request/accept`,
        null,
        {
          params: { order_id },
        }
  ...
  async cancelAdvanceBooking(order_id) {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/orders/advance/cancel`,
        null,
        {
          params: { order_id },
        }
  ...
  async rejectAdvanceBooking(order_id) {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/orders/advance/request/reject`,
        null,
        {
          params: { order_id },
        }
  ...
  async completeAdvanceBooking(order_id) {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/orders/advance/complete`,
        null,
        {
          params: { order_id },
        }
  ...
  listenToInstantBooking(chef_id, onMessage, onError) {
    const eventSource = new EventSource(
      `${API_BASE_URL}/orders/sse/instant-booking/${chef_id}`
    );
  ...
  async getCustomerById(id) {
    try {
      ...
      throw (
        error.response?.data || {
          success: false,
          message: `Error fetching customer with ID ${id}`,
        }
      );
    }
  },
  ...
  async signUpCustomer(signUpData) {
    try {
      ...
      throw (
        error.response?.data || {
          success: false,
          message: "Error during customer sign up",
        }
      );
    }
  },
  ...
  async fetchRecipeByChefIdAndRecipeId(chef_id, recipeId) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/recipes/${chef_id}/${recipeId}`
      );
      return response.data;
    } catch (error) {
